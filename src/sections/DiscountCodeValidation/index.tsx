...
interface DiscountCodeValidationProps {
  discountCode: string
}

const DiscountCodeValidation: React.FC<DiscountCodeValidationProps> = (props) => {
  ...
  const isInit = React.useMemo(() => value === EMPTY_STRING,
    [value])

  const colorName = React.useMemo(() => {
    if (isInit) return 'black'
  ...
  return (
    <div className='pb-6 relative'>
      <label
        ...
        className={`text-left block mb-2 text-sm font-medium text-${colorName}-700 dark:text-${colorName}-500`}
      >
        Event Discount Code
      </label>
      ...
      <input
        ...
        placeholder='Enter generated code here'
        onChange={handleChange}
      />

      {!isInit && (
        <p className={`absolute bottom-0 right-0 text-sm text-${colorName}-600 dark:text-${colorName}-500`}>
